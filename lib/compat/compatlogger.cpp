...
void CompatLogger::Start(bool runtimeCreated)
{
	...
	Checkable::OnNotificationSentToUser.connect([this](const Notification::Ptr& notification, const Checkable::Ptr& checkable,
		...
		const String& comment_text, const String& command_name, const MessageOrigin::Ptr&) {
		NotificationSentHandler(notification, checkable, user, type, cr, author, comment_text, command_name);
	});
